...
const PushNotificationManager: React.FC<Props> = ({ user }) => {
  ...
  const registerUnifiedPush = async () => {
    ...
    // Ativamos o VAPID para todos (incluindo Master) para redundância máxima.
    ...
    try {
      ...
      if (vapidSub) {
        // Se temos VAPID, ele é a estrutura principal de JSON (para todos, incluindo Master)
        combinedPayload = {
          ...vapidSub,
          fcmToken: fcmToken || undefined
        };
      } else if (fcmToken) {
        // Se só temos FCM, guardamos como string normal do token FCM
